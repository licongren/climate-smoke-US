...
scen_name <- "ssp370"
year_list <- 2025:2027#unique(fire_proj$year)
...
#fire_proj <-  readRDS(paste0(input_path,"/downscaled_emis_2025_2045_years.rds"))  %>% filter(scenario == scen_name) 
fire_proj <-  readRDS(paste0(input_path,"/downscaled_emis_10yr_2010_2090.rds"))  %>% filter(scenario == scen_name) 

##downscaled_emis_2046_2055_years.rds
#fire_proj <-  readRDS(paste0(input_path,"/downscaled_emis_final_subregion.rds")) %>% filter(scenario == scen_name) 
...
get_gfed_emis_monthwind <- function(grid_id=grid_id, fire_proj_data=fire_proj_data){
  ...
  fire_proj_tmp <- fire_proj_data %>%
    ...
    select(year, month, gcm, scenario, gfed_cell_id, pred_DM_grid, distance, angle)
  ...
  fire_era5_tmp <- fire_era5_tmp %>% pivot_longer(cols=pred_DM_grid) %>% 
    group_by(grid_id_10km, year, month, gcm, scenario, name, upwind) %>%
...
####### save data by 
#gcm_list <- unique(fire_proj$gcm)


for (yy in year_list){
  print(yy)
fire_proj_tmp <- fire_proj_month %>% filter(year == yy)
...
fire_proj_combine  <- foreach(ii=1:length(grid_10km_list),.options.snow = opts,.packages=c("dplyr","tidyr"),
                               ...
                               .init=list(list())) %dopar% {
                                 get_gfed_emis_monthwind(grid_id=grid_10km_list[ii], fire_proj_data=fire_proj_tmp)
                               }
stopCluster(cl)

fire_combine <- dplyr::bind_rows(fire_proj_combine[[1]])
#saveRDS(fire_combine, paste0(output_path,"/downscaled_emission/emis_proj_dist_wind_2050_10yrs_",scen_name,".rds"))
saveRDS(fire_combine, paste0(output_path,"/downscaled_emission/2025-2045/emis_proj_dist_wind_",yy,"_",scen_name,".rds"))
#saveRDS(fire_combine, paste0(output_path,"/downscaled_emission/emis_proj_dist_wind_2050_subregion_",scen_name,".rds"))
